...
const LocationCard: React.FC<LocationCardProps> = ({
  ...
  // Sync or fetch photos for this location
  useEffect(() => {
    ...
    const fetchLocationPhotos = async () => {
      try {
        const res = await fetch(`/api/photos?location=${encodeURIComponent(trip.location)}`);
        if (res.ok && isMounted) {
          const data: Photo[] = await res.json();
          setPhotos(data);
        }
      } catch (e) {
        console.error("Error loading location photos", e);
      }
    };
  ...
  const formatVacationDate = formatDate;

  const hasVacationDates =
    trip.vacationStart &&
    trip.vacationEnd &&
          ...
          <div className="mt-4 pt-3 border-t border-gray-200/60 flex items-center justify-between">
            {photos.length > 0 ? (
              <button
                ...
                className="flex items-center space-x-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold transition-all shadow-sm hover:shadow"
              >
            ...
            {isAuthenticated && (
              <button
                ...
                className="flex items-center space-x-1 px-2.5 py-1 bg-white hover:bg-gray-100 text-gray-700 border border-gray-300 rounded-lg text-xs font-medium transition-colors shadow-2xs"
                title="Add photo for this area"
              >
